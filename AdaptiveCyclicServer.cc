...
class AdaptiveCyclicServer : public cSimpleModule {
private:
...
public:
    AdaptiveCyclicServer();
    virtual~ AdaptiveCyclicServer();
protected:
...
Define_Module(AdaptiveCyclicServer);
...
void AdaptiveCyclicServer::initialize() {
...
void AdaptiveCyclicServer::handleMessage(cMessage *msg) {
    ...
    else if (msg->getFullName() == served) {
        gateInId = (msg->getKind() + 1) % numQueues;
        delete(msg);
