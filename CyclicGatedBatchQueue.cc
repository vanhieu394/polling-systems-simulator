...
public:
    CyclicGatedBatchQueue();
    virtual~ CyclicGatedBatchQueue();
protected:
...
Define_Module(CyclicGatedBatchQueue);

CyclicGatedBatchQueue::CyclicGatedBatchQueue() {
...
void CyclicGatedBatchQueue::handleMessage(cMessage *msg) {
...
//                << cycleNumber << "\n";
    ...
    // Serve the batch
        ...
        if (servLen >= batchSize)
            sizeOfCurrBatch = batchSize;
        else
            sizeOfCurrBatch = servLen;
